...
without frictionless metadata. All of these possibilities are illustrated in
...
describing the data to be deposited. The `deposits` package works with two
...
package](https://docs.ropensci.org/frictionless). Most of this present section
therefore concerns the first of these two kinds of metadata, with brief
demonstrations of the second kind used to illustrate how the frictionless
workflow integrates with the general `deposits` workflow. The term "metadata"
throughout the following should be understood to refer to the first of the
above kinds of metadata; the second kind is always referred to as "frictionless
metadata."

Metadata can be included in a deposits client in two main ways, either on
initial construction of a client, or through attaching metadata to an existing
client. One of the easiest ways to specify metadata is as a list of named terms
...
be assigned to any return value. Other ways of specifying and entering metadata
are described in [the introductory
vignette](https://docs.ropensci.org/deposits/articles/deposits.html).
