...
3. `deposit_fill_metadata(meta)` to fill a `deposits` client with metadata (see
   introductory vignette for details);
4. `deposit_new()` to create a new deposit on a deposits service using the
   metadata inserted with `fill_metadata()`;
5. `deposit_retrieve(deposit_id)` to retrieve information on a specified
   deposit;
