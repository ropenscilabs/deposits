...
the deposits client can then updated with [the `deposit_fill_metadata()`
...
An alternative procedure for updating metadata is described in the following
section.
...
computer, meaning that copies of these beaver datasets must be written to local
files.
...
Calling that method also writes a "frictionless" metadata file to the directory:
...
And that file contains all of the metadata in JSON format, along with
"frictionless" metadata describing the structures of the two datasets
themselves. The deposits metadata are contained within the "metadata" item,
which frictionless metadata are within the "resources" item. Once a
frictionless "datapackage.json" file has been generated, any subsequent editing
of metadata should be done by directly editing that file. Once edited, the
metadata in the client can then be updated like this:

```{r cli-update-meta, eval = FALSE}
cli$deposit_fill_metadata (beaver_dir)
```

The following code demonstrates by first modifying the "description" of that
file, and then showing that those changes are reflected in the client itself:
...
So local modifications to the metadata in the frictionless "datapackage.json"
file can be incorporated within the deposits client by calling [the
