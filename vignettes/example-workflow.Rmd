...
package](https://docs.ropensci.org/frictionless). Edited and updated versions
of metadata can then be loaded into a deposits client by passing the path to
the directory as the `path` argument to [the `deposit_fill_metadata()`
method](https://docs.ropensci.org/deposits/reference/depositsClient.html#method-deposit-fill-metadata-):
...
(The increase in sizes of the uploaded files there demonstrates that
...
The [`deposit_upload_file()`
method](https://docs.ropensci.org/deposits/reference/depositsClient.html#method-deposit-upload-file-)
can also be used to update files if they have been locally modified. This
requires setting an additional parameter of `overwrite = TRUE`, to force the
versions stored on the host service to be overwritten. Attempting to update
files by passing the `path` parameter only will pass the default `overwrite =
FALSE`, and will generate an error that the file already exists and will not be
overwritten.
