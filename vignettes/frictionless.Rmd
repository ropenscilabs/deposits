...
Without explicitly setting `overwrite = TRUE`, the existing version will not be
overwritten, and running that line will generate an error.
...
For demonstration purposes, the metadata in a `deposits` client may also be
directly edited and updated. For example, the metadata shown above may be
modified like this:
