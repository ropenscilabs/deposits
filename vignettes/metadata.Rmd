...
To restart an R session and connect to a local deposit, simply pass the path to
the local directory containing deposit data and a "datapackage.json" file to
the `new()` method:
...
"datapackage.json" file. If the deposit also exists only, any local changes to
"datapackage.json" will be reflected in changes on the remote deposits service,
including changes within the copy of "datapackage.json" held there.
