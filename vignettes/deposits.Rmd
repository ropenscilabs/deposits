...
different creators. For example,

```{r creator-example}
metadata <- list (
    # ... metadata plus ...
    creator = list (
        list (name = "A. Person", orcid = "0000-0001-0002-0003"),
        list (name = "B. Person", orcid = "0000-0004-0005-0006", email = "b.person@email.com")
    )
)
```
...
`deposits` client. A local file edited to hold identical metadata to that
entered above would look like this:
```{r metafile, echo = FALSE}
x <- '{
  "abstract": "This is the abstract",
  "created": "2023-01-01",
  "creator": [
...
writeLines (x, meta_file)
```
```{r metafile-lines}
readLines (meta_file)
```

The name of that file can then also be passed as the `metadata` parameter, so
that the following client will produce identical results to the previous
construction with metadata passed as an explicit list:
