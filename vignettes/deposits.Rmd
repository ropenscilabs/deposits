...
`deposits` client functions. The "metadata" item also includes an additional
"identifier" element containing a pre-reserved DOI provided by the deposits
service.
...
An example full workflow for creating and editing a deposits client and
...
3. Modify metadata. The following code provides a proof-of-principle
   modification of metadata, by changing "New Title" to "Updated Title":
    ...
    ```
    This is an indirect way of editing metadata, by using R code. The best way
    to update deposits metadata is to directly edit and modify the
