...
method to initiate a new client. An example full workflow to that point would
look something like the following four main steps:
...
2. Upload local data:
    ...
    cli$deposit_upload_file (path = path)
    ```
    That upload also creates local and remove versions of a frictionless
